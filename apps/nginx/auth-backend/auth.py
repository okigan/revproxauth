#!/usr/bin/env python3
"""
Simple RADIUS authentication backend for nginx auth_request module using FastAPI.
"""
...
RADIUS_NAS_IDENTIFIER = os.getenv("RADIUS_NAS_IDENTIFIER", "nginx-auth")
...
app = FastAPI(title="Nginx RADIUS Auth Backend")
...
async def login(next: str = "/", error: str = ""):
    ...
    <html>
    <head>
        <title>Login - Nginx RADIUS Auth</title>
        <style>
            body {{
    ...
    <body>
        <div class="login-box">
            <h1>🔐 Login</h1>
            <p style="color: #666; margin-bottom: 20px;">Nginx + RADIUS Authentication</p>
